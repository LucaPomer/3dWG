...
class Program {
    constructor(gl, config) {
        ...
        try { vertexShader = new Shader(gl, { type: gl.VERTEX_SHADER, code: config.codes[0] }) }
            ...
            util.fatal(`in vertex shader >${config.name}<: ` + ex)
        }
        let fragmentShader
        try { fragmentShader = new Shader(gl, { type: gl.FRAGMENT_SHADER, code: config.codes[1] }) }
        catch (ex) {
            util.fatal(`in fragment shader >${config.name}<: ` + ex)
        }
        ...
        if (!gl.getProgramParameter(this.glProgram, gl.LINK_STATUS)) {
            util.fatal(`linking program >${this.name}< failed: ${gl.getProgramInfoLog(this.glProgram)}`)
    ...
    setUniform(name, value) {
        ...
        if (!uniform) {
            util.warn(`uniform variable >${name}< not used in shader.`)
            return
        }
    ...
    setAttribute(name, buffer) {
        ...
        if (attribute && attribute.location >= 0) {
            ...
            util.warn(`vertex attribute >${name}< not used in vertex shader.`)
    }
    ...
    setTexture(name, unit, texture) {
        ...
        let uniform = this.uniforms[name]
        if (!uniform) {
            util.warn(`uniform sampler ${name} not used in shader.`)
            return
        }
